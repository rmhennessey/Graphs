...
def traverseMap(player, direction=''):
    ...
    # If all the rooms have been visited, then we are finished.
    ...
    # Else, use BFS to find the nearest room with an unexplored exit
        # travel there
        # SEE getAllSocialPaths from social.py project
        ...
        while q.size() > 0:
            ...
            if currentRoom not in visited:
                ...
                # For every direction in the current room:,
                    # add it to the path to search through 
                    # add it to the queue
    ...
    # For every room in path
    for r in travel_path:
        room = player.currentRoom.id
        graph_keys = graph[room].keys()

        # For every room visited
            # add the values that match to that room to our traversal path
        for t in graph_keys:
            if graph[room][t] == r:
                player.travel(t)
                traversalPath.append(t)
...
if len(visited_rooms) == len(roomGraph):
    print(
        f"SUCCESS: {len(traversalPath)} moves, {len(visited_rooms)} rooms visited")
else:
    print("GO FISH: INCOMPLETE TRAVERSAL")
